...
public class OutputAudioRecorder : MonoBehaviour
{
    ...
    public void StartRecording()
    {
        ...
        fileName = Path.Combine(@"C:\Users\felix.wikstrom\Desktop\RecordedSound", Path.GetFileNameWithoutExtension(FILENAME) + ".mp3");
        if (!recOutput)
        {
    ...
    private void StartWriting(String name)
    {
        ...
        string filePath = Path.Combine(@"C:\Users\felix.wikstrom\Desktop\RecordedSound", name);

        // Use FileMode.Create to create the file or overwrite if it already exists
